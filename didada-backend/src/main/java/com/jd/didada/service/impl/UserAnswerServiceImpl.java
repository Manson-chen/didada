...
public class UserAnswerServiceImpl extends ServiceImpl<UserAnswerMapper, UserAnswer> implements UserAnswerService {
    ...
    public void validUserAnswer(UserAnswer userAnswer, boolean add) {
        ...
        if (add) {
            // 补充校验规则
            ThrowUtils.throwIf(ObjectUtils.isEmpty(appType), ErrorCode.PARAMS_ERROR, "应用类型为空");
            ThrowUtils.throwIf(appId == null || appId < 0, ErrorCode.PARAMS_ERROR, "appId 非法");
            ThrowUtils.throwIf(ObjectUtils.isEmpty(userId), ErrorCode.PARAMS_ERROR, "用户id 为空");
            ThrowUtils.throwIf(ObjectUtils.isEmpty(choices), ErrorCode.PARAMS_ERROR, "用户答案选项为空");
        }
        // 修改数据时，有参数则校验
        // 补充校验规则
        if (appId != null) {
